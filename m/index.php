...
function get_streamer_data($arrs)
{
  ...
  $data['topic'] = preg_replace('/(https?|ftp)(:\/\/[[:alnum:]\+\$\;\?\.%,!#~*\/:@&=_-]+)/i', '<a class="urllink" href="\\0" target="_blank">■</a>' , $data['topic']);
...
*/
  ...
  $st_on = array_slice($streamer_data, $offset, $GLOBALS['page_size']);
...
/*
  ...
  $data = new Dwoo_Data();
  $data->assign('page', $p);
  $data->assign('random',time());
  $data->assign('page_num', $p_num);
  $data->assign('streamer_online_data', $contents_streamer);
  $data->assign('streamer_offline_data', '');
